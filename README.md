...
runs 309 checks. First run is a few minutes, mostly transfer; afterwards it
...
database, and an audit trail — the platform with the AI switched off. The
**Chat** tab switches it on when a key is present, and shows what was retrieved,
which query the model chose, the parameters the catalogue bound, and the chart
drawn from the rows that came back:
...
- **The dashboard has no automated tests.** The 309 checks cover the ETL, the
...
api/test_*.py     288 checks, no network, no API key
