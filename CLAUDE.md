...
309 checks across six suites, none of which need a network or an API key. If a
...
log; one that is, is. Stage 12 is unclaimed.
...
.venv/bin/python run_pipeline.py --check    # 309 checks, no network, no API key
